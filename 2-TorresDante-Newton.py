...
funciondiff: Derivada de nuestra funcion.
...
while respuesta == 1:
    ...
    functiondiff = sym.diff(function, x)
    print(localizar_raiz(function))
    print("Cual es la aproximacion que quieres utilizar?")
    aprox = float(input())
    ...
    while limit > error:
        # Se sustituye x y se divide la funcion entre la derivada de la funcion.
        result = aprox - ((function.subs(x, aprox)) / (functiondiff.subs(x, aprox)))
    ...
    print(f"Su resultado es {aprox.evalf()}")
