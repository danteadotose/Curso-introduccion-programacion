...
a: Limite superior de la ecuacion.
b: Limite inferior de la ecuacion.
...
while respuesta == 1:
    ...
    # Se determina mediante la formula conocida el numero que se va a utilizar como aproximacion
    aprox = (((a * function.subs(x, b)) - (b * function.subs(x, a))) / ((function.subs(x, b)) - (function.subs(x, a)))).evalf()
    # Se decide que valor se va a asignar a la variable K
    if (function.subs(x, a) * function.subs(x, aprox)) < 0:
        k = a
    else:
        k = b
    ...
    while limit > error:
        # Se determina el resultado mediante la formula concoida
        result = (((aprox * function.subs(x, k)) - (k * function.subs(x, aprox))) / ((function.subs(x, k)) - (function.subs(x, aprox)))).evalf()
